  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <Card>
          <SectionLabel>Unique Installs</SectionLabel>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 20 }}>
            <div style={{ fontSize: 36, fontWeight: 700, letterSpacing: '-1px', color: 'rgba(255,255,255,0.92)' }}>
              {fmt(uniqueEst.total)}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              { label: 'macOS', value: uniqueEst.mac, color: ACCENT },
              { label: 'Windows', value: uniqueEst.windows, color: '#60a5fa' },
              { label: 'Linux', value: uniqueEst.linux, color: '#34d399' },
            ].map(p => (
              <div key={p.label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>{p.label}</span>
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>
                    {uniqueEst.total > 0 ? Math.round((p.value / uniqueEst.total) * 100) : 0}%
                  </span>
                </div>
                <MiniBar value={p.value} max={uniqueMax} color={p.color} />
              </div>
            ))}
          </div>
        </Card>
      ...
      <Card style={{ display: 'flex', flexDirection: 'column', alignSelf: 'stretch' }}>
